												...
												//, double *init_guess)   // feel free to add a nonzero initial guess as needed
{
	...
	while (1) {
		...
		//line 6 rhs P1
		//cudaMemcpy(gpu_r, r, sizeof(double) * N, cudaMemcpyHostToDevice);		
		gpu_dotp_wshuffle<<<256,256>>>(gpu_r, gpu_r, N, gpu_res_norm_old);
		double res_norm = 1;
		...
		//line 6 rhs P2
		gpu_dotp_wshuffle<<<256,256>>>(gpu_p, gpu_Ap, N, gpu_pAp);
		double pAp = 0;
		cudaMemcpy(&pAp, gpu_pAp, sizeof(double), cudaMemcpyDeviceToHost);		
		
		double alpha = res_norm / pAp;
		...
		// line 9
		gpu_dotp_wshuffle<<<256,256>>>(gpu_r, gpu_r, N, gpu_res_norm_new);
