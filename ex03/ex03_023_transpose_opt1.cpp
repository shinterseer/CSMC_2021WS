...
int main(void)
{
  ...
  double *A, *cuda_A, *cuda_B;
	...
	for(int i = 0; i < int(sizes.size()); i++){
		...
		//elapsed_time = execution_wrapper((sizes[i]+255)/256, 256, 10, transpose_correct_opt1, cuda_A, cuda_B, sizes[i]);
		elapsed_time = execution_wrapper(256, 256, 10, transpose_correct_opt1, cuda_A, cuda_B, sizes[i]);
		...
		if (check_results){
			...
			cudaMemcpy(AT, cuda_B, sizes[i]*sizes[i]*sizeof(double), cudaMemcpyDeviceToHost);
